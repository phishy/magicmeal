...
export default function BloodPressureScreen() {
  ...
  const loadEntries = useCallback(async () => {
    try {
      ...
      console.error('Error loading blood pressure entries', error);
    }
  }, []);
  ...
  }, [systolic, diastolic, pulse, entries]);
  ...
  const chartData = useMemo(() => {
    ...
    const systolicNodes = recentEntries.map((entry, index) =>
      mapPoint(entry.systolic, index)
    );
    const diastolicNodes = recentEntries.map((entry, index) =>
      mapPoint(entry.diastolic, index)
    );
